...
import static com.lind.common.util.CharType.*;
...
public class TextUtility {
	...
	private static final String LAW_TAG = "条款项目";

	private static final String LAW_SKIP = "省市县人大常委高检法";
	...
	public static int charType(String str) {
		if (str != null && str.length() > 0) {
			if (CHINESE_NUMBERS.contains(str)) {
				return CT_CNUM;
				// 中文数字按照数字处理
				// return CT_NUM;
			}
			...
			if (ub1 < 128) {
				if (ub1 <= 32) {
					return CT_OTHER; // NON PRINTABLE CHARACTERS
				}

				// 左括号
				if (StringPool.LEFT_BRACKET.indexOf((char) b1) != -1) {
					return LEFT_BRACKET;
				}

				// 右括号
				if (StringPool.RIGHT_BRACE.indexOf((char) b1) != -1) {
				...
				/* 条款项目 */
				if (LAW_TAG.contains(str)) {
					return LAW_TYPE;
				}
				/* 省市县 */
				if (LAW_SKIP.contains(str)) {
					return LAW_FUZZY;
				}
				return CT_CHINESE;
