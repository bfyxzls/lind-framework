...
import com.lind.office.convert.utils.BaseUtils;
...
public class RemoteHtmlToWord {
	...
	public String excute(String pageUrl) throws Exception {
		String outputPath = new StringBuffer("/output/").append(BaseUtils.getDateStr("yyyyMMdd")).append("/word/")
				.append(BaseUtils.uuid2()).append(".doc").toString();
	...
	private String loadImgHttpSrcToHtml(String pageUrl, String htmlText) {
		...
		while (m_img.find()) {
			...
			if (BaseUtils.isBlank(imgSrc) || imgSrc.startsWith("http")) {
				...
				String img_Tag = m_img.group().replaceAll("src\\s*=\\s*[\"|']{1}?([^\"|^']*?)[\"|']{1}",
						"src=\"" + imgSrc + "\"");
				// *** 匹配<img>中的src数据,并替换为本地服务http的地址
