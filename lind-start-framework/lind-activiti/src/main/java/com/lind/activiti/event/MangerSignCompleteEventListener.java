...
/**
 * 会签完成 解决ioc注入问题，在activiti配置时使用代理表达式${mangerSignCompleteEventListener}
 */
@Component
@Transactional
@Slf4j
public class MangerSignCompleteEventListener implements TaskListener {
	...
	public void notify(DelegateTask delegateTask) {
		log.info("会签审批启动");
		if (delegateTask.getVariable(Constant.completeCount) != null
				&& delegateTask.getVariable(Constant.allTaskCount) != null) {
