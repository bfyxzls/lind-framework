...
public interface Trace<S, T> {
	...
	/**
	 * This callback is called each time a trigger is fired, before evaluation, allowing
	 * to trace all events sent to the sate machine
	 * @param trigger Trigger sent to the state machine
	 */
	void trigger(T trigger);

	/**
	 * This callback is called each time a transition is performed, after trigger
	 * evaluation.
	 * @param trigger Trigger sent to the state machine
