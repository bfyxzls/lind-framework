package com.lind.common.core.stateless4j.conversion;
...
public final class ParameterConversion {
	...
	public static Object unpack(Object[] args, Class<?> argType, int index) {
		...
		if (arg != null && !argType.isAssignableFrom(arg.getClass())) {
			throw new IllegalStateException(
					String.format("The argument in position %s is of type %s but must be of type %s.", index,
							arg.getClass(), argType));
		}
	...
	public static void validate(Object[] args, Class<?>[] expected) {
		if (args.length > expected.length) {
			throw new IllegalStateException(String.format(
					"Too many parameters have been supplied. Expecting %s but got %s.", expected.length, args.length));
		}
