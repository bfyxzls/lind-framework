...
public class RegexUtil {
	...
	public static String hideOtherText(String myText) {
		...
		rtnText = RegExUtils.replaceAll(rtnText,
				"([，。,；;,][\\d一二三四五六七八九零０１２３４５６７８９]+年[\\d一二三四五六七八九零０１２３４５６７８９]+月[\\d一二三四五六七八九零０１２３４５６７８９]+日出?生)([，。,；;,])",
				"$1▲$2");
		rtnText = RegExUtils.replaceAll(rtnText, "([，。,；;,])(家?住[^房院])", "$1▲$2");
		rtnText = RegExUtils.replaceAll(rtnText, "([，。,；;,])((联系|注册)地)", "$1▲$2");
		rtnText = RegExUtils.replaceAll(rtnText, "([，。,；;,])([男女][，。,；;,])", "$1▲$2");
		String rplText = "";
		rtnText = RegExUtils.replaceAll(rtnText, "([，。,；;,][^，。,；;,▲]*▲[^。]*)", rplText);
