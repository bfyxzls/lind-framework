...
import com.lind.common.core.stateless4j.delegates.Func2;
...
public class StateMachineConfig<TState, TTrigger> {
	...
	/**
	 * Return StateRepresentation for the specified state. Creates representation if it
	 * does not exist.
	 * @param state The state
	...
	/**
	 * Begin configuration of the entry/exit actions and allowed transitions when the
	 * state machine is in a particular state
	...
	private void saveTriggerConfiguration(TriggerWithParameters<TTrigger> trigger) {
		if (triggerConfiguration.containsKey(trigger.getTrigger())) {
			throw new IllegalStateException(
					"Parameters for the trigger '" + trigger + "' have already been configured.");
		}
	 ...
	 * @return An object that can be passed to the fire() method in order to fire the
	 * parameterised trigger
	 */
	public <TArg0> TriggerWithParameters1<TArg0, TTrigger> setTriggerParameters(TTrigger trigger,
			Class<TArg0> classe0) {
	 ...
	 * parameterised trigger
	 */
	public <TArg0, TArg1> TriggerWithParameters2<TArg0, TArg1, TTrigger> setTriggerParameters(TTrigger trigger,
			Class<TArg0> classe0, Class<TArg1> classe1) {
		TriggerWithParameters2<TArg0, TArg1, TTrigger> configuration = new TriggerWithParameters2<>(trigger, classe0,
				classe1);
		saveTriggerConfiguration(configuration);
		return configuration;
	}
	 ...
	 * parameterised trigger
	 */
	public <TArg0, TArg1, TArg2> TriggerWithParameters3<TArg0, TArg1, TArg2, TTrigger> setTriggerParameters(
			TTrigger trigger, Class<TArg0> classe0, Class<TArg1> classe1, Class<TArg2> classe2) {
		TriggerWithParameters3<TArg0, TArg1, TArg2, TTrigger> configuration = new TriggerWithParameters3<>(trigger,
	...
	public void generateDotFileInto(final OutputStream dotFile, boolean printLabels) throws IOException {
		try (OutputStreamWriter w = new OutputStreamWriter(dotFile, "UTF-8")) {
			...
			for (Map.Entry<TState, StateRepresentation<TState, TTrigger>> entry : this.stateConfiguration.entrySet()) {
				Map<TTrigger, List<TriggerBehaviour<TState, TTrigger>>> behaviours = entry.getValue()
						...
						if (triggerBehaviour instanceof TransitioningTriggerBehaviour) {
							...
							if (printLabels) {
								writer.write(String.format("\t%s -> %s [label = \"%s\" ];\n", entry.getKey(),
										destination, triggerBehaviour.getTrigger()));
							}
							else {
