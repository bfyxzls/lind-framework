...
import com.lind.common.core.stateless4j.delegates.*;
import com.lind.common.core.stateless4j.transitions.Transition;
import com.lind.common.core.stateless4j.transitions.TransitioningTriggerBehaviour;
import com.lind.common.core.stateless4j.triggers.*;
...
public class StateConfiguration<S, T> {
	...
	public StateConfiguration(final StateRepresentation<S, T> representation,
			final Func2<S, StateRepresentation<S, T>> lookup) {
	 ...
	 * Accept the specified trigger and transition to the destination state.
	 * <p>
	 * Additionally a given action is performed when transitioning. This action will be
	 * called after the onExit action of the current state and before the onEntry action
	...
	/**
	 * Accept the specified trigger and transition to the destination state if guard is
	 * true
	 * @param trigger The accepted trigger
	 * @param destinationState The state that the trigger will cause a transition to
	 ...
	 * <p>
	 * Applies to the current state only. No exit or entry actions will be executed and
	 * the state will not change. The only thing that happens is the execution of a given
	 * action.
	 * @param trigger The accepted trigger
	 ...
	 * action.
	 * <p>
	 * The action is only executed if the given guard returns true. Otherwise this
	 * transition will not be taken into account (so it does not count as 'ignore', then).
	 * @param trigger The accepted trigger
	 * @param guard Function that must return true in order for the trigger to be accepted
	 * @param action The action to be performed
	 * @return The receiver
	 ...
	 * Reentry behaves as though the configured state transitions to an identical sibling
	 * state
	 * <p>
	 * Applies to the current state only. Will not re-execute superstate actions, or cause
	 * actions to execute transitioning between super- and sub-states
	 * @param trigger The accepted trigger
	 * @return The receiver
	 */
	 ...
	 * <p>
	 * Additionally a given action is performed when transitioning. This action will be
	 * called after the onExit action and before the onEntry action (of the re-entered
	 * state).
	 * @param trigger The accepted trigger
	 * @param action The action to be performed "during" transition
	 * @return The receiver
	...
	/**
	 * ignore the specified trigger when in the configured state, if the guard returns
	 * true
	 * @param trigger The trigger to ignore
	...
	/**
	 * Specify an action that will execute when transitioning into the configured state
	 * @param entryAction Action to execute, providing details of the transition and
	 * trigger parameters
	 * @return The receiver
	 */
	...
	/**
	 * Specify an action that will execute when transitioning into the configured state
	 * @param trigger The trigger by which the state must be entered in order for the
	 * action to execute
	 * @param entryAction Action to execute
	 * @return The receiver
	 */
	 ...
	 * @return The receiver
	 */
	public <TArg0> StateConfiguration<S, T> onEntryFrom(TriggerWithParameters1<TArg0, T> trigger,
	...
	@Deprecated
	public <TArg0> StateConfiguration<S, T> onEntryFrom(TriggerWithParameters1<TArg0, T> trigger,
			final Action1<TArg0> entryAction, final Class<TArg0> classe0) {
	...
	@Deprecated
	public <TArg0> StateConfiguration<S, T> onEntryFrom(TriggerWithParameters1<TArg0, T> trigger,
			final Action2<TArg0, Transition<S, T>> entryAction, final Class<TArg0> classe0) {
		return onEntryFrom(trigger, entryAction);
	}
	 ...
	 * @return The receiver
	 */
	public <TArg0, TArg1> StateConfiguration<S, T> onEntryFrom(TriggerWithParameters2<TArg0, TArg1, T> trigger,
	...
	@Deprecated
	public <TArg0, TArg1> StateConfiguration<S, T> onEntryFrom(TriggerWithParameters2<TArg0, TArg1, T> trigger,
			final Action2<TArg0, TArg1> entryAction, final Class<TArg0> classe0, final Class<TArg1> classe1) {
		return onEntryFrom(trigger, entryAction);
	}
	...
	@Deprecated
	public <TArg0, TArg1> StateConfiguration<S, T> onEntryFrom(TriggerWithParameters2<TArg0, TArg1, T> trigger,
			final Action3<TArg0, TArg1, Transition<S, T>> entryAction, final Class<TArg0> classe0,
	...
	public <TArg0, TArg1, TArg2> StateConfiguration<S, T> onEntryFrom(
			TriggerWithParameters3<TArg0, TArg1, TArg2, T> trigger, final Action3<TArg0, TArg1, TArg2> entryAction) {
		assert entryAction != null : ENTRY_ACTION_IS_NULL;
	...
	public <TArg0, TArg1, TArg2> StateConfiguration<S, T> onEntryFrom(
			TriggerWithParameters3<TArg0, TArg1, TArg2, T> trigger, final Action3<TArg0, TArg1, TArg2> entryAction,
			final Class<TArg0> classe0, final Class<TArg1> classe1, final Class<TArg2> classe2) {
		return onEntryFrom(trigger, entryAction);
	}
	 ...
	 */
	public <TArg0, TArg1, TArg2> StateConfiguration<S, T> onEntryFrom(
			TriggerWithParameters3<TArg0, TArg1, TArg2, T> trigger,
	...
	public <TArg0, TArg1, TArg2> StateConfiguration<S, T> onEntryFrom(
			TriggerWithParameters3<TArg0, TArg1, TArg2, T> trigger,
			final Action4<TArg0, TArg1, TArg2, Transition<S, T>> entryAction, final Class<TArg0> classe0,
			final Class<TArg1> classe1, final Class<TArg2> classe2) {
		return onEntryFrom(trigger, entryAction);
	}
	 ...
	 * <p>
	 * Substates inherit the allowed transitions of their superstate. When entering
	 * directly into a substate from outside of the superstate, entry actions for the
	 * superstate are executed. Likewise when leaving from the substate to outside the
	 * supserstate, exit actions for the superstate will execute.
	 ...
	 * @param trigger The accepted trigger
	 * @param destinationStateSelector Function to calculate the state that the trigger
	 * will cause a transition to
	 * @return The receiver
	 */
	 ...
	 * Additionally a given action is performed when transitioning. This action will be
	 * called after the onExit action and before the onEntry action (of the re-entered
	 * state). The parameter of the trigger will be given to this action.
	 * @param trigger The accepted trigger
	 * @param destinationStateSelector Function to calculate the state that the trigger
	 * will cause a transition to
	 * @param action The action to be performed "during" transition
	 ...
	 * @return The receiver
	 */
	public <TArg0, TArg1> StateConfiguration<S, T> permitDynamic(TriggerWithParameters2<TArg0, TArg1, T> trigger,
	 ...
	 * Additionally a given action is performed when transitioning. This action will be
	 * called after the onExit action and before the onEntry action (of the re-entered
	 * state). The parameters of the trigger will be given to this action.
	 * @param trigger The accepted trigger
	 * @param destinationStateSelector Function to calculate the state that the trigger
	 * will cause a transition to
	 * @param action The action to be performed "during" transition
	...
	public <TArg0, TArg1, TArg2> StateConfiguration<S, T> permitDynamic(
			TriggerWithParameters3<TArg0, TArg1, TArg2, T> trigger,
			final Func4<TArg0, TArg1, TArg2, S> destinationStateSelector, final Action3<TArg0, TArg1, TArg2> action) {
		return permitDynamicIf(trigger, destinationStateSelector, NO_GUARD, action);
	}
	 ...
	 * @return The receiver
	 */
	public StateConfiguration<S, T> permitDynamicIf(T trigger, final Func<S> destinationStateSelector,
			FuncBoolean guard) {
	 ...
	 */
	public StateConfiguration<S, T> permitDynamicIf(T trigger, final Func<S> destinationStateSelector,
			FuncBoolean guard, final Action action) {
		assert destinationStateSelector != null : DESTINATION_STATE_SELECTOR_IS_NULL;
		return publicPermitDynamicIf(trigger, new Func2<Object[], S>() {
			@Override
			public S call(Object[] arg0) {
	 ...
	 * @return The receiver
	 */
	public <TArg0> StateConfiguration<S, T> permitDynamicIf(TriggerWithParameters1<TArg0, T> trigger,
			final Func2<TArg0, S> destinationStateSelector, FuncBoolean guard) {
		assert trigger != null : TRIGGER_IS_NULL;
		assert destinationStateSelector != null : DESTINATION_STATE_SELECTOR_IS_NULL;
	 ...
	 * Additionally a given action is performed when transitioning. This action will be
	 * called after the onExit action of the current state and before the onEntry action
	 * of the destination state. The parameter of the trigger will be given to this
	 * action.
	 * @param trigger The accepted trigger
	 * @param destinationStateSelector Function to calculate the state that the trigger
	 * will cause a transition to
	 ...
	 */
	public <TArg0> StateConfiguration<S, T> permitDynamicIf(TriggerWithParameters1<TArg0, T> trigger,
			final Func2<TArg0, S> destinationStateSelector, FuncBoolean guard, final Action1<TArg0> action) {
		assert trigger != null : TRIGGER_IS_NULL;
		assert destinationStateSelector != null : DESTINATION_STATE_SELECTOR_IS_NULL;
		return publicPermitDynamicIf(trigger.getTrigger(), new Func2<Object[], S>() {
			@SuppressWarnings("unchecked")
	 ...
	 * @return The receiver
	 */
	public <TArg0, TArg1> StateConfiguration<S, T> permitDynamicIf(TriggerWithParameters2<TArg0, TArg1, T> trigger,
			final Func3<TArg0, TArg1, S> destinationStateSelector, FuncBoolean guard) {
		assert trigger != null : TRIGGER_IS_NULL;
		assert destinationStateSelector != null : DESTINATION_STATE_SELECTOR_IS_NULL;
		return publicPermitDynamicIf(trigger.getTrigger(), new Func2<Object[], S>() {
			@SuppressWarnings("unchecked")
	 ...
	 * Additionally a given action is performed when transitioning. This action will be
	 * called after the onExit action of the current state and before the onEntry action
	 * of the destination state. The parameters of the trigger will be given to this
	 * action.
	 * @param trigger The accepted trigger
	 * @param destinationStateSelector Function to calculate the state that the trigger
	 * will cause a transition to
	...
	void enforceNotIdentityTransition(S destination) {
		if (destination.equals(representation.getUnderlyingState())) {
			throw new IllegalStateException(
					"Permit() (and PermitIf()) require that the destination state is not equal to the source state. To accept a trigger without changing state, use either ignore(), permitInternal() or permitReentry().");
		}
	}
	...
	StateConfiguration<S, T> publicPermitIf(T trigger, S destinationState, FuncBoolean guard, Action action) {
		...
		assert guard != null : GUARD_IS_NULL;
		representation
				.addTriggerBehaviour(new TransitioningTriggerBehaviour<>(trigger, destinationState, guard, action));
		return this;
	}
	...
	StateConfiguration<S, T> publicPermitDynamicIf(T trigger, Func2<Object[], S> destinationStateSelector,
			FuncBoolean guard) {
		assert destinationStateSelector != null : DESTINATION_STATE_SELECTOR_IS_NULL;
		assert guard != null : GUARD_IS_NULL;
		representation.addTriggerBehaviour(
				new DynamicTriggerBehaviour<>(trigger, destinationStateSelector, guard, NO_ACTION_N));
		return this;
	}

	StateConfiguration<S, T> publicPermitDynamicIf(T trigger, Func2<Object[], S> destinationStateSelector,
			FuncBoolean guard, Action1<Object[]> action) {
		assert destinationStateSelector != null : DESTINATION_STATE_SELECTOR_IS_NULL;
		assert guard != null : GUARD_IS_NULL;
		representation
				.addTriggerBehaviour(new DynamicTriggerBehaviour<>(trigger, destinationStateSelector, guard, action));
		return this;
	}
