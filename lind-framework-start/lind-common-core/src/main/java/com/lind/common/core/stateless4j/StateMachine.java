...
public class StateMachine<S, T> {
	...
	/**
	 * Construct a state machine with external state storage. Assumes that external
	 * storage has the initial state available
	 * @param stateAccessor State accessor
	 * @param stateMutator State mutator
	...
	/**
	 * Fire initial transition into the initial state. All super-states are entered too.
	 *
	...
	/**
	 * Transition from the current state via the specified trigger. The target state is
	 * determined by the configuration of the current state. Actions associated with
	 ...
	 */
	public <TArg0, TArg1, TArg2> void fire(TriggerWithParameters3<TArg0, TArg1, TArg2, T> trigger, TArg0 arg0,
	 ...
	 * Override the default behaviour of throwing an exception when an unhandled trigger
	 * is fired
	 * @param unhandledTriggerAction An action to call with state, trigger and params when
	 * an unhandled trigger is fired
	 */
	...
	/**
	 * Set tracer delegate. Set trace delegate to investigate what the state machine is
	 * doing at runtime. Trace delegate will be called on {@link #fire(Object)} and on
	 * transition.
	...
	public String toString() {
		...
		return String.format("StateMachine {{ State = %s, PermittedTriggers = {{ %s }}}}", getState(),
				params.toString());
	}
