...
ORYX.Core.SVG.Label = Clazz.extend({
	...
	_checkFittingToReferencedElem: function() {
		try {
			...
			//if (/Firefox[\/\s](\d+\.\d+)/.svt(navigator.userAgent) && new Number(RegExp.$1)>=3) {
				var newtspans = [];
...
//svt string: abcdefghijklmnopqrstuvwxyz????????????????,.-#+ 1234567890?????ABCDEFGHIJKLMNOPQRSTUVWXYZ;:_'*???????????????!"????$%&/()=?[]{}|<>'~????`\^?????????@?????????????????
...
ORYX.Core.StencilSet.Rules = {
	...
	outgoingEdgeStencils: function(args) {
		...
		// svt each edge, if it can connect to source
...
//    },
    ...
    /**
     * Performs the cut controller by first copy-ing and then deleting the
     * current selection.
     */
    editCut: function(){
    ...
    /**
     * Performs the delete controller. No more asking.
     */
    editDelete: function(){
        var selection = this.facade.getSelection();
