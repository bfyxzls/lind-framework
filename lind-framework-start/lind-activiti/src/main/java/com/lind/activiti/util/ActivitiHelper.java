package com.lind.activiti.util;
...
public class ActivitiHelper {
	 ...
	 * @return
	 */
	public List<TaskDefinition> nextTaskDefinitionList(ActivityImpl activityImpl, String activityId, String elString,
		...
		if ("userTask".equals(activityImpl.getProperty("type")) && !activityId.equals(activityImpl.getId())) {
			...
			if (activityImpl.getActivityBehavior() instanceof ParallelMultiInstanceBehavior) {
				ParallelMultiInstanceBehavior activityBehavior = (ParallelMultiInstanceBehavior) activityImpl
						.getActivityBehavior();
			...
			for (PvmTransition tr : outTransitions) {
				...
				if ("exclusiveGateway".equals(ac.getProperty("type"))) {
					...
					if (outTransitionsTemp.size() == 1) {
						return nextTaskDefinitionList((ActivityImpl) outTransitionsTemp.get(0).getDestination(),
					...
					if (activity.getActivityBehavior() instanceof UserTaskActivityBehavior) {
						...
						ParallelMultiInstanceBehavior activityBehavior = (ParallelMultiInstanceBehavior) activity
								.getActivityBehavior();
						UserTaskActivityBehavior userTaskActivityBehavior = (UserTaskActivityBehavior) activityBehavior
								.getInnerActivityBehavior();
						taskDefinition = userTaskActivityBehavior.getTaskDefinition();
					...
					if (outTransitionsTemp.size() == 1) {
						...
						for (PvmTransition tr1 : outTransitionsTemp) {
							List<TaskDefinition> taskDefinitionList = nextTaskDefinitionList(
									(ActivityImpl) tr1.getDestination(), activityId, elString, processInstanceId);
			...
			for (PvmTransition tr : outTransitions) {
				...
				if ("exclusiveGateway".equals(ac.getProperty("type"))) {
					...
					if (activityBehavior instanceof MultiInstanceActivityBehavior) {
						return ((UserTaskActivityBehavior) ((MultiInstanceActivityBehavior) activityBehavior)
								.getInnerActivityBehavior()).getTaskDefinition();
					}
					return ((UserTaskActivityBehavior) activityBehavior).getTaskDefinition();
				}
			}
			return null;
		}
	...
	public Boolean isEndNode(@PathVariable String procInstId) {
		...
		ProcessDefinitionEntity dfe = (ProcessDefinitionEntity) ((RepositoryServiceImpl) repositoryService)
