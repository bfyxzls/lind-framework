...
public class DiskFile implements Closeable {
	...
	public static class BlockMeta implements Comparable<BlockMeta> {
		...
		/**
		 * Only used for {@link SeekIter} to seek a target block meta. we only care about
		 * the lastKV, so the other fields can be anything.
	...
	private class InternalIterator implements MStore.SeekIter<KeyValue> {
		...
		public void seekTo(KeyValue target) throws IOException {
			...
			if (blockMetaIter.hasNext()) {
				currentReader = load(blockMetaIter.next());
				currentKVIndex = 0;
				// Locate the smallest KV which is greater than or equals to the given KV.
				// We're sure that
				// we can find the currentKVIndex, because lastKV of the block is greater
				// than or equals
				// to the target KV.
