...
/*
	这段代码的作用是将字节数组this.buffer中从offset位置开始的4个字节解析为一个大端序的无符号整数，并将其对一个非负整数this.modDiviso取模后返回。
...
这里需要解释两个概念：
    ...
    "大端序" (Big-Endian)：表示将数值的高位字节存放在内存的低地址处，而将数值的低位字节存放在内存的高地址处。
    "无符号整数" (Unsigned integer)：整数是没有符号（+/-）的数值，也就是说它只能表示正数和零。

举个例子，以下是十进制数1890278377和对应的16进制表示70FC0109，在大端序的无符号整数表示中它们是一致的：

    70（十六进制）对应二进制 01110000
    FC（十六进制）对应二进制 11111100
    01（十六进制）对应二进制 00000001
    09（十六进制）对应二进制 00001001
...
因此，大端序的无符号整数采取的是从左到右的排列方式，最左边的字节包含了整数的最高有效位，最右边的字节包含了整数的最低有效位。
 ...
 @Test
	public void daduanxu(){
	 ...
	 int result= ((buffer[offset] & 0x7f) << 24 |
			 (buffer[offset + 1] & 0xff) << 16 |
			 (buffer[offset + 2] & 0xff) << 8 |
			 (buffer[offset + 3] & 0xff)) % modDiviso;
